...
if [ ! -f /data/app-storage.img ]; then
    echo "Creating 100GB sparse image..."
    dd if=/dev/zero of=/data/app-storage.img bs=1 count=0 seek=100G
    mkfs.ext4 /data/app-storage.img
fi
...
mount -o loop /data/app-storage.img /mnt/app-storage
