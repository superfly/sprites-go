...
func (s *System) StartProcess() error {
	...
	// Store the process with proper locking
	s.processMu.Lock()
	...
	if pidFileWatcher != nil {
		...
		case result := <-pidFileWatcher.wait():
			...
			if result.err != nil {
				...
				s.processStarted.Store(false) // Reset the atomic flag
			...
			go func() {
				...
				// Signal process stopped by swapping channels
				s.processMu.Lock()
		...
		case <-processExited:
			...
			s.processCmd = nil           // Clear the process command
			s.processMu.Unlock()
			s.processStarted.Store(false) // Reset the atomic flag
...
func (s *System) generateCrashReport(err error, processRuntime time.Duration, stdoutBuffer, stderrBuffer *tap.CircularBuffer) {
	// Skip crash reporting if this is a graceful shutdown
	if s.gracefulShutdown {
		s.logger.Info("Skipping crash report for graceful shutdown")
		return
	}
...
func (s *System) StopProcess() error {
	...
	if s.processCmd == nil || s.processCmd.Process == nil {
		// Reset the atomic flag even if no process is running
		s.processStarted.Store(false)
		return nil
	}

	// Mark this as a graceful shutdown to skip crash reporting
...
func (s *System) WaitForProcess() error {
