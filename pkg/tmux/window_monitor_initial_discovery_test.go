...
func TestInitialDiscoveryLinksExistingExecWindow(t *testing.T) {
    ...
    // Wait for the info log "Starting initial window discovery"
    foundLog := false
    deadline := time.Now().Add(2 * time.Second)
    for time.Now().Before(deadline) {
        entries := tap.GetLogBuffer().Snapshot(200, slog.LevelInfo, nil)
        for _, e := range entries {
            if e.Message == "Starting initial window discovery" {
                foundLog = true
                break
            }
        }
    ...
    if !foundLog {
        t.Fatal("Did not observe 'Starting initial window discovery' log entry")
    }
