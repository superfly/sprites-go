...
// CheckpointInfoWithStats extends checkpoint info with statistics
...
func (j *JuiceFS) ListCheckpointsWithStats(ctx context.Context) ([]CheckpointInfoWithStats, error) {
	// Build a minimal list by inspecting filesystem; DB-backed listing has moved out of juicefs
	// We will synthesize only IDs and timestamps based on filesystem where possible
	checkpoints := make([]CheckpointInfoWithStats, 0)
	...
	for i, cp := range checkpoints {
		cpWithStats := CheckpointInfoWithStats{ID: cp.ID, CreateTime: cp.CreateTime}
