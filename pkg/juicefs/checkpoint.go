...
func (j *JuiceFS) Checkpoint(ctx context.Context, checkpointID string) error {
	...
	record, err := j.checkpointDB.CreateCheckpoint(
		...
		func(src, dst string) error {
			...
			cloneStart := time.Now()
			cloneBase := os.Getenv("JUICEFS_CLONE_CMD")
			var cmdName string
			var cmdArgs []string
			if cloneBase == "" {
				cmdName = "juicefs"
				cmdArgs = []string{"clone", srcPath, dstPath}
			} else {
				parts := strings.Fields(cloneBase)
				cmdName = parts[0]
				if len(parts) > 1 {
					cmdArgs = append(cmdArgs, parts[1:]...)
				}
				cmdArgs = append(cmdArgs, srcPath, dstPath)
			}
			cloneCmd := exec.CommandContext(ctx, cmdName, cmdArgs...)
...
func (j *JuiceFS) Restore(ctx context.Context, checkpointID string) error {
	...
	if len(entries) == 0 {
		...
		if cloneBase == "" {
			cmdName = "juicefs"
			cmdArgs = []string{"clone", fullCheckpointPath, activeDir}
		} else {
			parts := strings.Fields(cloneBase)
			cmdName = parts[0]
			if len(parts) > 1 {
			...
			cmdArgs = append(cmdArgs, fullCheckpointPath, activeDir)
		}
		cloneCmd := exec.CommandContext(ctx, cmdName, cmdArgs...)
