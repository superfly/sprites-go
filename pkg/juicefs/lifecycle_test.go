...
func TestStopIsIdempotent(t *testing.T) {
	...
	// Start in background (will fail without juicefs binary, but that's OK)
	...
	// Wait a moment for Start() to actually begin
	time.Sleep(50 * time.Millisecond)

	// Multiple Stops should all succeed (idempotent)
	...
	case <-time.After(1 * time.Second):
		t.Log("Start() goroutine did not finish within 1 second")
	}
}
